...
public class AudioProcessor implements IAVCall<AVMsg>{
    ...
    private boolean audioEncodeStep(boolean isEnd){
        Log.e("wuwang","audioEncodeStep:"+isRecordAudioStarted);
        if(isRecordAudioStarted){
            int inputIndex=mAudioEncoder.dequeueInputBuffer(1000);
            Log.e("wuwang","audioEncodeStep inputIndex:"+inputIndex);
            if(inputIndex>=0){
                ...
                Log.e("wuwang","store---> audioSize:"+mTempInfo.size);
            ...
            while (true){
                ...
                Log.e("wuwang","store----> ss :"+outputIndex);
                if(outputIndex>=0){
                    //todo 第一帧音频时间戳为0的问题
                    if(mMediaStore!=null&&isStoreStarted){
                        ...
                        mHardCodecData.info=mTempInfo;
                        Log.e("wuwang","store----> tryAudio :"+mHardCodecData.trackIndex);
                    ...
                    Log.e("wuwang","get audio output format changed ->"+mAudioEncoder.getOutputFormat().toString());
                    if(mMediaStore!=null){
    ...
    public void onCall(AVMsg data) {
        Log.e("wuwang","audio onCall:"+data.msgStr);
        if(data.msgType==AVMsg.TYPE_INFO){
            switch (data.msgRet){
                case AVMsg.MSG_CMD_START:
                    ...
                    startCodec();
                    isCmdStarted=true;
                    Log.e("wuwang","isRecordAudioStarted--->CMD_START:"+isRecordAudioStarted);
                    break;
                case AVMsg.MSG_CMD_END:
                    isCmdStarted=false;
                    try {
                        Log.e("wuwang","try to stop audio");
                        mThread.join();
                        Log.e("wuwang","audio stoped");
        ...
        try {
            ...
            mThread=new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    Log.e("wuwang","audioEncodeStep ture before");
                    audioEncodeStep(true);
                    Log.e("wuwang","audioEncodeStep ture aff");
                    stopCodec();
                }
            });
            mThread.start();
