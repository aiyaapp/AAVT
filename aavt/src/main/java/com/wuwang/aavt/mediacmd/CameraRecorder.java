...
public class CameraRecorder {
    ...
    public CameraRecorder(String data){
        ...
        //用于音频采集
        RecordAudioActuator mAudio=new RecordAudioActuator(mMuxer);
        ...
        if(data.equals("Camera")){
            ...
            Mp4Extractor mMp4Actuator=new Mp4Extractor();
            mMp4Actuator.setDataSource(data);
            process.setSuccessor(mMp4Actuator);
            process.setAsCamera(false);
        }
        surfaceShow.setSuccessor(process);
        surfaceEncode.setSuccessor(surfaceShow);
        mAudio.setSuccessor(surfaceEncode);

        this.mProcessor=mAudio;
    }
